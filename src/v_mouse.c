...
{
	*(unsigned long *)&pb->contrl[9] = (*v->mouseapi->setvector)( MVEC_BUT, *(unsigned long *)&pb->contrl[7]);
}

void
vex_curv( VDIPB *pb, VIRTUAL *v)
{
	*(unsigned long *)&pb->contrl[9] = (*v->mouseapi->setvector)( MVEC_CUR, *(unsigned long *)&pb->contrl[7]);
}
	
void
vex_motv( VDIPB *pb, VIRTUAL *v)
{
	*(unsigned long *)&pb->contrl[9] = (*v->mouseapi->setvector)( MVEC_MOV, *(unsigned long *)&pb->contrl[7]);
}
