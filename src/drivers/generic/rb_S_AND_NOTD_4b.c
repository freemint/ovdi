...
void _cdecl rb_S_AND_NOTD_4b(ROP_PB *rpb);

void _cdecl
rb_S_AND_NOTD_4b(ROP_PB *rpb)
{
						...
						*d-- = (*d & ~begmask) | (((p3 << shift) & ~*d) & begmask);
						for (dwords = swords; dwords > 1; dwords--)
						{
							...
							*d-- = ((p0 >> rshift) | (tmp << shift)) & ~*d;
							p0 = tmp;
							tmp = *s--;
							*d-- = ((p1 >> rshift) | (tmp << shift)) & ~*d;
							p1 = tmp;
							tmp = *s--;
							*d-- = ((p2 >> rshift) | (tmp << shift)) & ~*d;
							p2 = tmp;
							tmp = *s--;
							*d-- = ((p3 >> rshift) | (tmp << shift)) & ~*d;
							p3 = tmp;
						}
						...
						*d-- = (*d & ~begmask) | ( (((p3 >> shift) | (*s << rshift)) & ~*d) & begmask);
						...
						for (swords = dwords; swords > 1; swords--)
						{
							...
							*d-- = ((p0 >> shift) | (tmp << rshift)) & ~*d;
							p0 = tmp;
							tmp = *s--;
							*d-- = ((p1 >> shift) | (tmp << rshift)) & ~*d;
							p1 = tmp;
							tmp = *s--;
							*d-- = ((p2 >> shift) | (tmp << rshift)) & ~*d;
							p2 = tmp;
							tmp = *s--;
							*d-- = ((p3 >> shift) | (tmp << rshift)) & ~*d;
							p3 = tmp;
						}
