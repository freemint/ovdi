...
lvst_font( VIRTUAL *v, short id)
{
	...
	if (sr == 1)
	{
		...
		v->font.scaled = 0;
		v->font.sclsts = 0;
		get_font_chrfx_ofst(sf, v->font.style, &v->font.fx_x_ofst, &v->font.fx_y_ofst);
...
vqt_name( VDIPB *pb, VIRTUAL *v)
{
	...
	if (index)
	{
		if (v->font.loaded)
		{
			index = find_fontbyindex( v->font.loaded, index, (long *)&f);
			if (index)
				f = v->fring;
		}
		else
